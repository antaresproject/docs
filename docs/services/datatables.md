...
<pre><code>protected function rowActions($row)
{
    $this->tableActions = [];
    $html               = app('html');
...
This is a definition of the '**html**' method, which is responsible for table's prototype preparation (including mass actions). Within the implementation the '**addMassAction**' method is placed which (in an argument) receives the reference to action where the identifiers (multiple ids) of the marked rows are send. It is good to notice the parameterization of the reference. The result of the code above:
  ...
  ![AT_DATAS05](/png/docs/services/datatables/AT_DATAS05.png)
...
<pre><code>return $this->setName('Foo List')
            ->addColumn(['data' => 'description', 'name' => 'description', 'title' => trans('antares/foo::datagrid.header.description'), 'searchable' => false]);</code></pre>
...
<pre><code>Event::listen('datatables:admin/users/index:column.id', function(&$attributes) {
...
<pre><code>Event::listen('datatables:filters.admin/users/index.userstatusfilter.before', function($filter) {
...
<pre><code>Event::listen('datatables.value.admin/users/index', function($datatables) {
